	...
	for(unsigned i=1; i<half_edges.size(); ++i)
	{
		...
		else
		{
			...
			{								//if it fails, most likely the input data are messed up
				...
				if (half_edges[i] == half_edges[i+1]) {
					std::cout << "edge vertices 0 " << half_edges[i-1].vertex_0 << " " << half_edges[i].vertex_0 << " " << half_edges[i+1].vertex_0 << std::endl;
					std::cout << "edge vertices 1 " << half_edges[i-1].vertex_1 << " " << half_edges[i].vertex_1 << " " << half_edges[i+1].vertex_1 << std::endl;
				}
